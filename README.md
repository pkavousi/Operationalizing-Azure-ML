...
* A machine learning production model is generated by AutoML in Azure Machine Learning Studio, and then its best model is deployed. Swagger UI is used to import swagger.json file of the deployed endpoint and Apache Bench (ab) is used to benchmark the deployed model.  
* The second part of the project repeats the same steps by Azure Python SDK to create, train, and publish a pipeline. The provided Jupyter notebook is used for this stepFor this part.

The used dataset is available from [here](https://automlsamplenotebookdata.blob.core.windows.net/automl-sample-notebook-data/bankmarketing_train.csv) and contains marketing data about individuals. The data is for a marketing campaign (phone calls) with the the output as whether the client will subscribe a bank term deposit.
...
The best model of the AutoML run is deployed The _Best Model_ will appear in the _Details_ tab, while it will appear first in the _Models_ tab. An HTTP weblink is generated for its endpoint.  

**Enable Logging:**
The application insight is ebaled and a log is generated for the deployed model.

**Swagger Documentation:**
Azure generates a Swagger JSON file for the deployed model. Swagger UI is used to consume the endpoint and provide us with some insights into model input structure.

**Consume Model Endpoints:**
I used the ```endpoint.py``` script to interact with the deployed model. I run the script with the _scoring_uri_that was generated after deployment and -since I enabled Authentication- the_key_of the service. This URI is found in the_Details_ tab, above the Swagger URI.
...
I enabled _Application Insights_ and be able to retrieve logs:
...
We can run the _logs.py_ Python file, where  _name_ is the name of the deployed model (_deployedmodel_) and I add the line `service.update(enable_app_insights=True)`:  
...
The  _swagger.sh_ and _serve.py_ download and run the latest Swagger Docker container in (_swagger.sh_), and start a Python server on port 9000 (_serve.py_).
...
Replacin Explore field with : `http://localhost:9000/swagger.json`, Swagger UI generates interactive API documentation that provides the opportunity to make API calls directly in the browser.  
...
Apache Bench is use to benchmark the deployed endpoint. It provides load testing and benchmarking tool for Hypertext Transfer Protocol (HTTP) server.
![run benchmark.sh.py](images/Consume2.JPG?raw=true "Endpoint Benchmarking")
...
![Jupyter Notebook: RunDetails Widget shows the step runs](images/Pipeline2.JPG.JPG?raw=true "Jupyter Notebook: RunDetails Widget shows the step runs")
...
* The data is **highly imbalanced** and an auxilary method like SMOTE could make the prediction better. Furthermore, a single XGboost or Catboost along with Hyperparameter optimization might give better results that autoML. It is worth trying. 
